...
def calc_psnr(sr, hr, scale, rgb_range, dataset=None):
    ...
    if sr.size(2) != hr.size(2):  # zxy, if the size of sr is different with the hr image, delete them.
        size2 = int(hr.size(2) / scale) * scale
        sr = sr[:, :, :size2 - sr.size(2), :]
        hr = hr[:, :, :size2 - hr.size(2), :]
    if sr.size(3) != hr.size(3):
        size3 = int(hr.size(3) / scale) * scale
        sr = sr[:, :, :, :size3 - sr.size(3)]
        hr = hr[:, :, :, :size3 - hr.size(3)]
        # sr = sr[:, :, :size2 - sr.size(2), :size3 - sr.size(3)]
        # print('sr1:', sr.shape)
        # print('hr1:', hr.shape)
...
def make_optimizer(args, target):
    ...
    if args.optimizer == 'SGD':
        ...
        kwargs_optimizer['eps'] = args.epsilon
    elif args.optimizer == 'ADAN':  # zxy, 在ADAN里把参数改了保持一致
        # kwargs_optimizer['opt-eps'] = args.opt_eps
        # kwargs_optimizer['opt-betas'] = args.opt_betas
        # kwargs_optimizer['max-grad-norm'] = args.max_grad_norm
        # kwargs_optimizer['no-prox'] = args.no_prox
        # param = trainable
        optimizer_class = Adan
        # optimizer_class = Adan(trainable, lr=args.lr, weight_decay=args.weight_decay, betas=args.opt_betas, eps=args.opt_eps,
...
def calc_ssim(X, Y, scale, rgb_range, dataset=None, sigma=1.5, K1=0.01, K2=0.03, R=255):
    ...
    if dataset and dataset.dataset.benchmark:
        ...
        if X.size(1) > 1:
            ...
            if X.size(2) != Y.size(2):  # zxy, if the size of sr is different with the hr image, delete them.
                size2 = int(Y.size(2) / scale) * scale
                X = X[:, :, :size2 - X.size(2), :]
                Y = Y[:, :, :size2 - Y.size(2), :]
            if X.size(3) != Y.size(3):
                size3 = int(Y.size(3) / scale) * scale
                X = X[:, :, :, :size3 - X.size(3)]
                Y = Y[:, :, :, :size3 - Y.size(3)]
            X = X.mul(convert).sum(dim=1)  # sr大
            Y = Y.mul(convert).sum(dim=1)  # hr小
    else:
        shave = scale + 6
